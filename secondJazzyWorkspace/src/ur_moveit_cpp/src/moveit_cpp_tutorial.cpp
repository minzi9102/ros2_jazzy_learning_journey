...
#include <geometry_msgs/msg/point_stamped.h>//<depend>geometry_msgs</depend>
...
int main(int argc, char** argv)
{
  ...
  if (plan_solution1)
  {
    ...
    visual_tools.publishText(text_pose, "plan1:set start pose as current pose", rvt::WHITE, rvt::XLARGE);
    ...
    /* bool blocking = true; */
    // moveit_controller_manager::ExecutionStatus result = moveit_cpp_ptr->execute(plan_solution1.trajectory, CONTROLLERS);
  }
  ...
  geometry_msgs::msg::Pose start_pose;//<depend>geometry_msgs</depend>
  ...
  if (plan_solution2)
  {
    ...
    visual_tools.publishAxisLabeled(target_pose1.pose, "target_pose");
    visual_tools.publishText(text_pose, "plan2: set start pose as other pose, move to last goal ", rvt::WHITE, rvt::XLARGE);
    // visual_tools.publishTrajectoryLine(plan_solution2.trajectory, joint_model_group_ptr);
    visual_tools.trigger();

    /* Uncomment if you want to execute the plan */
    /* bool blocking = true; */
    moveit_cpp_ptr->execute(plan_solution2.trajectory, CONTROLLERS); 
  }
  ...
  geometry_msgs::msg::Pose target_pose2;//<depend>geometry_msgs</depend>
  ...
  planning_components->setGoal("test_configuration");
  ...
  moveit_msgs::msg::CollisionObject collision_object;//<depend>moveit_msgs</depend>
  collision_object.header.frame_id = "tool0";
  collision_object.id = "box";

  shape_msgs::msg::SolidPrimitive box;//<depend>shape_msgs</depend>
  box.type = box.BOX;
  ...
  planning_components->setStartStateToCurrentState();
  planning_components->setGoal("up");
