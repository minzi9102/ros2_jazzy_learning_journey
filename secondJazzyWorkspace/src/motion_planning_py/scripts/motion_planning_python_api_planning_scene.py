...
def main():
    ...
    ur = MoveItPy(node_name="moveit_py_planning_scene")
    ur_manipulator = ur.get_planning_component("ur_manipulator")
    planning_scene_monitor = ur.get_planning_scene_monitor()
    ...
    with planning_scene_monitor.read_only() as scene:
        ...
        robot_state.set_from_ik("ur_manipulator", pose_goal, "tool0")
        robot_state.update()  # required to update transforms
        robot_collision_status = scene.is_state_colliding(
            robot_state=robot_state, joint_model_group_name="ur_manipulator", verbose=True
        )
