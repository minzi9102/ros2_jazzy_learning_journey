...
def plan_and_execute(
    ...
    # execute the plan
    if plan_result:
        logger.info("正在执行已规划路径Executing plan")
        robot_trajectory = plan_result.trajectory
        robot.execute(robot_trajectory, controllers=[])
    else:
        logger.error("规划失败Planning failed")
...
def main():
    ...
    logger.info("MoveItPy instance created#MoveitPy实例已生成")
    ...
    # plan to goal
    # plan_and_execute(ur, ur_manipulator, logger, single_plan_parameters=plan_params, sleep_time=3.0)
