...
- 调通了ur_planning_scene_ros_api的代码，并成功迁移为ur机械臂版本
- 可以在场景中新增物品，并且可以让机械臂带着物体运动
...
ros2 launch ur_robot_driver ur_control.launch.py \
      ur_type:=ur3 \
终端#2
cd /ros2_workspaces/four_workspace
source install/setup.bash
ros2 launch ur_planning_scene_ros_api ur_moveit2.launch.py ur_type:=ur3
终端#3
cd /ros2_workspaces/four_workspace
source install/setup.bash
ros2 launch ur_planning_scene_ros_api planning_scene_ros_api_tutorial.launch.py
```

#### 日期：2025/10/22

- 在ur机械臂上跑通了motion planning api的代码
    - 是通过修改ur_control.launch.py来启动所有必须的依赖程序的
    - 大致理清了launch脚本的运作顺序和原理

- 接下来准备学习motion planning pipeline代码
- humble版本的moveit tutorial的github仓库没有正确的pipeline代码，通过从最新分支中复制了cpp、launch.py、cmakelist并重新编译，还要修改moveit tutorial包最外层的cmake，取消注释`add_subdirectory(doc/examples/motion_planning_pipeline)`才能正常运行

#### 日期：2025/10/30

- 开始学习moveitcpp的代码
- 能在虚拟panda机械臂上跑通，但是在第五个plan时没有按代码计划出现一个方块
- 尝试迁移到ur机械臂上之后再修改代码
- 在four_workspace上创建一个程序包
```
cd src
ros2 pkg create --build-type ament_cmake ur_moveit_cpp
```
- 完善CMakeList.txt、package.xml
...
```
- 完善launch文件
    - 复制了ur_motion_planning_api包里的ur_control.launch.py,并做出修改
    - 复制了ur_motion_planning_api包里的ur_motion_planning_api.launch.py,并做出修改
- 复制launch\rviz配置文件，config\ur3_kinematic.yaml配置文件
- 修改cpp文件
- 编译试运行
- 不太行
...
- 尝试切换到jazzy版本的ros重新进行moveit_cpp的迁移
- 这样可以使用moveit config builder 的新特性，可能会更方便
- jazzy不支持`moveit-task-constructor-core`二进制安装，必须通过源码编译，有点麻烦
- `fatal: repository 'https://github.com/Kinovarobotics/ros2_kinova.git/' not found`
- 本地源码编译`moveit-task-constructor-core`和`ros2_kinova`，成功了
- 编译moveit2_tutorial,也成功了

#### 日期：2025/11/03
#### 日期：2025/11/05

- 重新学习jazzy版本的moveit tutorial，重点关注代码中参数传递过程
- 从最开始的robot model and robot state开始学习
- 创建程序包ur_model_and_state
  
  ```
  ros2 pkg creat --build-type ament_cmake ur_model_and_state
  ```
- 在src文件夹中完善cpp文件，复制原来的cpp文件，将`getJointModelGroup`改为`ur_manipulator`

- 创建程序包的流程
  1. 完成cpp，确定所需传入的参数
  2. 完成launch，确定所需要的依赖项
  3. 完成package.xml
  4. 完成cmakelist.txt find_package,add_executable,安装TARGET(executable),DIRECTORY(launch,config),FILES(package.xml)

#### 日期2025/11/11

  - 成功了！能够将motion_planning_py接口应用在ur机械臂上，但是还没进行实机测试
  - 先分析了ur_control.launch.py的逻辑，他启动了哪些节点？这些节点怎样配合工作？启动脚本传入了哪些参数？这些参数怎样传入节点？
  - 再分析ur_moveit_config.launch.py的逻辑
  - 再分析motion_planning_py.launch.py的逻辑
  - 理解了moveitConfigBuilder的逻辑（moveit_config.todict()会包含哪些配置？）
  - 搭配ur_control.launch.py和自定义ur_motion_planning_py.launch.py就能正常进入motion_planning_py
  - ur_motion_planning_py.launch.py只启动static_tf和motion_planning_py节点（自行配置moveitconfig，还要搭配自己的motion_planning_python_api_tutorial.yaml）
  - 具体脚本倒是没什么难的
  - 现在存在的问题：
    1. 规划的动作幅度很大，实机会有碰撞风险，可以增加一个规划确认安全后再执行的功能
    2. 规划会进入超出限制的构型（提示起始位置超出限制），需要研究问题原因是什么
    3. 不能使用visual_tools来实现逐步推进代码，难以观察
  - 下一步的计划：改回moveit_cpp接口，启动逻辑应该跟moveit_py差不多

#### 日期：2025/11/14
  - 创建`ur_moveit_cpp`程序包
  ```
  ros2 pkg create --build-type ament_cmake ur_moveit_cpp
  ```
  - 复制cpp文件
    - 在vscode中安装ros拓展（原版已弃用，跟指示安装新版）
    - 在顶部命令窗口中执行`ROS2: Update C++ Properties`
  - 创建launch文件夹和launch文件
  - 配置config文件夹：包括moveit_cpp.yaml,visual.rviz
  - 修改launch文件对应配置
  - 修改package.xml
  - 修改cmakelist.txt
  - 编译
